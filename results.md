...
The graphical representation image, which represents a graph with all airports and connections, is implemented in the “PrintFullMap” function. This "PrintFullMap" function also has some commented code that allows the user to visualize any airports that have no connections to any other airport. Also, the full map function only needs to be run once, and may be commented out after that. Another graphical representation image called “printMST”, which represents the Minimum Spanning Tree of a certain source, is implemented in the dijkstra’s algorithm. This function has the same implementation as the “PrintFullMap” function, except it iterates through a newly created adjacency list made when our dijkstra's algorithm was called, and it colors the source red and destination green. Finally, “printSinglePath” is the function that has the same implementation as the printMST function, where it iterates through the adjacency list of the MST. However, this function uses a helper function that searches through the MST to find the path from the source node to the destination node that are defined at the calling of the function. This path is then returned in a vector so the "printSinglePath" function is able to iterate through this path and highlight the edges(blue) and intermediate nodes (yellow) on a new PNG. The "printSinglePath" function can be found in the ‘Project’ class in the corresponding ‘.cpp’ and ‘.h’ files in the ‘src’ folder. 

The full map .png image is titled “graph.png”. The MST for a single node has its own .png image that is called “source” + “destination”.png. The final .png is generated displaying the shortest path of the two nodes. This image is called “source” + “destination” + "singlepath”.png. All of these images will be saved into the main, root project directory. 
...
Although this work is not explicitly written in our project proposal, the assumption that our programs work as planned requires the need for test suites. If we wrote implementations without testing, our program might be faulty and if used in the real world, would cause further issues. It is also important to note that any images that are created when running the test suites, will be saved to the root directory. Each of the files will have descriptive names that inform on what they are testing. Also, to whomever runs the code on their machine, please do not remove any files, because they may have been hardcoded files that will be used for testing.
